// for this suite to work, the AllJoyn Discovery mock service with PIN code
// authentication must be running
// how to run it? after running npm install:
// > <project_root>/build/Release/sample-mock-device-discovery-pin
...
describe('An AllJoyn about announcement', function() {
  ...
  it('should call AuthListener\'s requestCredentials after secureConnectionAsync is called', function() {
    assert.equal(authListenerRequestCredentialsWasCalled, true);
  });
  it('should call AuthListener\'s authenticationComplete with success after requestCredentials is called', function() {
    assert.equal(authListenerAuthenticationCompleteWasCalled, true);
    assert.equal(sessionfulData.authSuccess, true);
  });
});
