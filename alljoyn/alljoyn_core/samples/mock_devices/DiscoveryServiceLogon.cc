...
class MyAuthListner: public AuthListener {
  qcc::String username = "admin";
  qcc::String password = "password";
  ...
  bool RequestCredentials(const char* authMechanism, const char* authPeer, uint16_t authCount, const char* userId, uint16_t credMask, Credentials& creds) {
    ...
    if (strcmp(authMechanism, "ALLJOYN_SRP_LOGON") == 0) {
  		if (credMask & AuthListener::CRED_PASSWORD && credMask & AuthListener::CRED_LOGON_ENTRY) {
				if (strcmp(userId, username.c_str()) == 0) {
...
public:
  ...
  QStatus CreateInterface(BusAttachment& bus, const InterfaceDescription** pIntf)
  {
  	...
  	QStatus status = bus.CreateInterface(INTERFACE_NAME, testIntf, AJ_IFC_SECURITY_REQUIRED);
